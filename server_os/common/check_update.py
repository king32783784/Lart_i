...
class Check_Update(ReadPublicinfo):
    ...
    def getisoname(self):
        ...
        if len(targetiso) != 0:
            return targetiso[-1] + '.iso'
        else:
            return targetiso  #need report this error  to tester by mail

    def getisomd5(self):
        xmlurl = os.path.join(self.setup['xml_dict']['isourl'][0], self.isoname) + '.md5sum'
        print xmlurl
        remode = "(.+)"
    ...
    def downloadiso(self):
        remode = "href=\"(.+).iso\">"
        print self.setup['xml_dict']['isoserver'][0]
        isoname = self.get_htmlcontent(self.setup['xml_dict']['isourl'][0], remode)
        locatedir = os.path.join(self.setup['xml_dict']['isoserver'][0], self.isoname)
        isourl = os.path.join(self.setup['xml_dict']['isourl'][0], self.isoname)
