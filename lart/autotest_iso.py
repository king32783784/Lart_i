...
'''
    解析配置文件，"test_setup_info"是个字典，其中'xml_list'对应配置列表
    'xml_dict'是一个以配置项为键，其对应设置内容为值的字典"
'''
...
if __name__ == "__main__":
    daemon = Check_iso_update(test_setup_info['xml_dict']['isourl'],
                              test_setup_info['xml_dict']['checkfreq
                              uency'])
    if len(sys.argv) == 2:
        if 'start' == sys.argv[1]:
            daemon.start()
